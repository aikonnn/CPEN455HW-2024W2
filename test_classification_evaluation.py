...
def classifier(model, data_loader, device):
    ...
    for batch_idx, item in enumerate(tqdm(data_loader)):
        ...
        for filename, pred in zip(img_path, answer):
            result_rows.append({'file': filename, 'class': pred.item()})
    result_df = pd.DataFrame(result_rows)
    result_df[0] = result_df[0].str.replace("data/", "", regex=False)
    result_df.to_csv('result/result.csv', index=False, header=False)
